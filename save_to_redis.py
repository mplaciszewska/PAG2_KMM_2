...
def save_to_redis(redis_db, station_collection, INPUT_stations, INPUT_year, INPUT_month):
    ...
    # iterate though all parameter types
    for i, dataframe in enumerate(dataframes.values()):
        parameter = variables_names[i]
        if dataframe is None or dataframe.empty:
            continue

        # iterate through all INPUT_stations (in the INPUT_powiat)
        for station in INPUT_stations:
            station_data = {}
            
            station_code = station_collection.find_one({'ifcid': station})['ifcid']
            ...
            # iterate through all days in the INPUT_month
            for day in range(1, days_in_INPUT_month.days+1):
                ...
                # save to a redis has with a key of 'station_value__month'
